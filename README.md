...
уведомления о нём приходят по разу, от лица команды с меньшим id. Исключение —
мультикиллы: четвёрка игрока каждой команды это отдельный хайлайт, и приходят
обе. Разбор — в [docs/architecture.md](docs/architecture.md).

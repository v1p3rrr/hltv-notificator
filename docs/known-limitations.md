...
## Тумблеры типов уведомлений на команду не работают

Колонка `teams.muted_events` заведена и есть методы `set_team_mutes` /
`team_mutes`, но нигде не проверяется: заглушить отдельный тип события для
отдельной команды пока нельзя. Неочевидная часть этой задачи — матч двух
отслеживаемых команд с разными настройками: разумное правило «шлём, если хочет
хотя бы одна», иначе одна команда молча глушила бы уведомления про другую.

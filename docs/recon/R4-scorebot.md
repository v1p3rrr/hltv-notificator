...
Хуже того: **в `log` при подключении приезжает бэклог уже случившегося**. На
записи матча `2397053` (серия из двух сыгранных карт) насчиталось **10 событий
`MatchStarted`** — по нескольку на каждую карту, включая карту, которая
завершилась задолго до подключения:

```
MatchStarted maps: [de_dust2 ×6, de_mirage ×4]
```
